...
exports.getPapers = async (req, res, next) => {
    try {
        const { page = 1, limit = 20, category, sort = "latest", startDate, endDate, q, search } = req.query;
        let sql = `SELECT p.id, p.arxiv_id, p.title_en, p.title_ko, p.primary_category,
                   p.categories, p.published_at as published_date, p.pdf_url, p.arxiv_url, p.view_count, ps.tldr, ps.one_line_summary
                   FROM papers p LEFT JOIN paper_summaries ps ON p.id = ps.paper_id WHERE 1=1`;
        const params = [];

        // 검색어 처리 (q 또는 search 파라미터)
        const searchQuery = (q || search || '').trim();
        if (searchQuery) {
            sql += ` AND (p.title_en LIKE ? OR p.title_ko LIKE ? OR p.arxiv_id LIKE ?)`;
        ...
        // 정렬 옵션 매핑: 프론트엔드 값 → SORT_OPTIONS 키
        const sortMapping = {
            'latest': 'LATEST',
            'oldest': 'OLDEST',
            'popular': 'MOST_VIEWED',
        ...
        const sortKey = sortMapping[sort.toLowerCase()] || 'LATEST';
        const orderBy = SORT_OPTIONS[sortKey] || SORT_OPTIONS.LATEST;
        const result = await paginate(sql, params, { page: parseInt(page), limit: Math.min(parseInt(limit), 100), orderBy });
        res.json({ success: true, data: { papers: result.items.map(formatPaperListItem) }, pagination: result.pagination });
...
exports.getTodayPapers = async (req, res, next) => {
    try {
        ...
        const papers = await query(`SELECT p.id, p.arxiv_id, p.title_en, p.title_ko, p.primary_category,
            p.categories, p.published_at as published_date, p.pdf_url, ps.tldr, ps.one_line_summary
...
exports.getTrendingPapers = async (req, res, next) => {
    try {
        ...
        const papers = await query(`SELECT p.id, p.arxiv_id, p.title_en, p.title_ko, p.primary_category,
            p.published_at as published_date, ps.tldr, ps.one_line_summary FROM papers p
...
exports.getMonthlyStats = async (req, res, next) => {
    try {
        ...
        const params = [];
        if (category) { sql += ` AND primary_category = ?`; params.push(category); }
        sql += ` GROUP BY DATE_FORMAT(published_at, '%Y-%m') ORDER BY month DESC`;
        ...
        if (category) { categorySql += ` AND primary_category = ?`; categoryParams.push(category); }
        categorySql += ` GROUP BY DATE_FORMAT(published_at, '%Y-%m'), primary_category ORDER BY month DESC, count DESC`;
        const categoryStats = await query(categorySql, categoryParams);

        // 월별로 카테고리 그룹화
        const monthlyCategories = {};
        categoryStats.forEach(stat => {
            if (!monthlyCategories[stat.month]) {
                monthlyCategories[stat.month] = [];
            }
            monthlyCategories[stat.month].push({
                category: stat.primary_category,
                name: PAPER_CATEGORIES[stat.primary_category]?.name || stat.primary_category,
                nameKo: PAPER_CATEGORIES[stat.primary_category]?.nameKo || stat.primary_category,
                count: stat.count
        ...
        const result = stats.map(stat => ({
            month: stat.month,
            year: parseInt(stat.month.split('-')[0]),
            monthNum: parseInt(stat.month.split('-')[1]),
            count: stat.count,
            firstDate: stat.first_date,
            lastDate: stat.last_date,
            categories: monthlyCategories[stat.month] || []
        }));
...
exports.getPaperById = async (req, res, next) => {
    try {
        const { id } = req.params;
        let paper = await queryOne(`SELECT p.*, ps.tldr, ps.one_line_summary, ps.summary_3line, ps.summary_detailed,
            ps.business_insight FROM papers p
...
exports.getPaperSummary = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: { tldr: summary.tldr, summary3Line: summary.summary_3line,
            detailedSummary: summary.summary_detailed, businessInsight: summary.business_insight,
            titleKo: summary.title_ko, abstractKo: summary.abstract_ko }
        });
    } catch (error) { next(error); }
};
...
exports.recordView = async (req, res, next) => {
    try {
        const { id } = req.params;
        await query("UPDATE papers SET view_count = view_count + 1 WHERE id = ?", [id]);

        // 로그인한 사용자의 경우 읽기 기록 저장
        if (req.user) {
            try {
                // 기존 기록이 있으면 업데이트, 없으면 삽입
                const existing = await queryOne(
                    "SELECT id FROM reading_history WHERE user_id = ? AND paper_id = ?",
                    [req.user.id, id]
                );

                if (existing) {
                    await query(
                        "UPDATE reading_history SET read_at = CURRENT_TIMESTAMP WHERE id = ?",
                        [existing.id]
                    );
                } else {
                    await query(
                        "INSERT INTO reading_history (user_id, paper_id) VALUES (?, ?)",
                        [req.user.id, id]
                    );
                }
            } catch (historyError) {
                // 읽기 기록 저장 실패해도 조회수 기록은 성공으로 처리
                console.error('Reading history error:', historyError.message);
            }
        }

        res.json({ success: true, message: "조회수가 기록되었습니다." });
    } catch (error) {
        // 조회수 증가 실패해도 에러 반환하지 않음
        res.json({ success: true, message: "조회수가 기록되었습니다." });
    }
};
...
function formatPaperListItem(paper) {
    return { id: paper.id, arxivId: paper.arxiv_id, title: paper.title_ko || paper.title_en,
        ...
        publishedDate: paper.published_date, tldr: paper.tldr,
        oneLineSummary: paper.one_line_summary,
        pdfUrl: paper.pdf_url, arxivUrl: paper.arxiv_url,
...
function formatPaperDetail(paper) {
    return {
        ...
        abstract_ko_google: paper.abstract_ko_google,
        ...
        updatedDate: paper.updated_at_arxiv,
        ...
        summary: {
            ...
            three_line: paper.summary_3line,
            threeLine: paper.summary_3line,
            detailed: paper.summary_detailed,
            businessInsight: paper.business_insight,
            business_insight: paper.business_insight
        },
        translation_tier: "haiku",
        translationTier: "haiku",
        githubUrls: paper.github_urls ? (typeof paper.github_urls === 'string' ? JSON.parse(paper.github_urls) : paper.github_urls) : null,
        pwcUrl: paper.pwc_url
    };
}
